...
void create_instance(char ** instance_extensions, uint32_t instance_extension_count)
{
	...
	if (vkal_info.enable_validation_layers) {
		for (int i = 0; i < array_length(vkal_info.validation_layers); ++i) {
			layer_ok = check_validation_layer_support(vkal_info.validation_layers[i], available_layer_names, layer_count);
			if (!layer_ok) {
				printf("validation layer not available: %s\n", vkal_info.validation_layers[i]);
				DBG_VULKAN_ASSERT(VK_ERROR_LAYER_NOT_PRESENT, "requested validation layer not present");
			}
		}
	}
	if (layer_ok) {
		create_info.enabledLayerCount = array_length(vkal_info.validation_layers);
		create_info.ppEnabledLayerNames = vkal_info.validation_layers;
	}
...
{
	vkal_info.nv_rt_ctx.properties = { VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PROPERTIES_NV };
...
void flush_command_buffer(VkCommandBuffer command_buffer, VkQueue queue, bool free)
{
	...
	VkFence fence;
	DBG_VULKAN_ASSERT(vkCreateFence(vkal_info.device, &fence_info, nullptr, &fence), "failed to create fence");
	...
	vkDestroyFence(vkal_info.device, fence, nullptr);
...
void create_rt_storage_image()
{
	...
	// upload using staging buffer
	{
		...
		flush_command_buffer(cmd_buf, vkal_info.graphics_queue, true);
	}
}
...
void build_rt_commandbuffers()
{
	...
	for (int i = 0; i < vkal_info.swapchain_image_count; ++i) {
		...
		copy_region.extent = { VKAL_RT_SIZE_X, VKAL_RT_SIZE_Y, 1 };
		vkCmdCopyImage(
			vkal_info.nv_rt_ctx.command_buffers[i],
...
void build_rt_acceleration_structure(VkGeometryNV * geometry, uint32_t geometryNV_count, VkBuffer instance_buffer)
{
	...
	flush_command_buffer(command_buffer, vkal_info.graphics_queue, true);
}
...
void create_render_to_image_render_pass()
{
	...
	static const VkAttachmentReference color_attachment_refs[] =
	{
		{
	...
	static const VkAttachmentReference depth_attachment_ref =
	{
		1,
		VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
	};
...
void create_render_pass()
{
	...
	// ImGui Renderpass
	{
		...
		// Create the Render Pass
		{
			...
			err = vkCreateRenderPass(vkal_info.device, &info, nullptr, &vkal_info.imgui_render_pass);
